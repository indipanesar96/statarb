...
class Cointegrator:
    ...
    def run_cointegrator(self, clustering_results: Dict[int, Tuple[str]]) -> Dict[int, Tuple[str]]:
        ...
        for cluster in clustering_results.values():
            for pair in cluster:
                ...
                if adf_test_statistic <= adf_critical_values \
                        ['5%'] and hl_test <= self.max_mean_rev_time and hurst_exp <= 0.5:
                    # stock1_holding = 0
                    # stock2_holding = 0
                    # stock_holding_list = []
    ...
    def hurst_exponent_test(self, residuals):
        ...
        results = LinearRegression().fit(np.log10(tau_vector).reshape(-1, 1),
                                         np.log10(variance_delta_vector).reshape(-1, 1))
